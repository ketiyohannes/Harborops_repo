...
request_nonce="$(python3 -c 'import uuid; print(uuid.uuid4())')"
session_signature="$(python3 -c 'import hashlib,hmac,sys; print(hmac.new(sys.argv[1].encode("utf-8"), "\n".join([sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5]]).encode("utf-8"), hashlib.sha256).hexdigest())' "${csrf_token}" "POST" "/api/auth/favorites/" "${request_ts}" "${request_nonce}")"
